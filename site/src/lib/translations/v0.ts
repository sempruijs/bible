...
const BOOK_NAME_TO_BIBLE_BOOK: Record<string, BibleBook> = {
  ...
  "SONG OF SONGS": BibleBook.SongOfSongs,
...
export const toTranslation = (translationV0: TranslationV0): Effect.Effect<Translation, BookNameParseError> =>
  Effect.forEach(
    translationV0.books,
    (bookV0: Book) =>
      Effect.map(
        parseBookName(bookV0.name),
        (bibleBook): TranslationBook => ({
          name: bibleBook,
    ...
    Effect.map((books): Translation => ({ books }))
  );
